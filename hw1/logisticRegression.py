...
def compute_avglogL(X,y,beta):
    ...
    for i in range(n):
        avglogL += (y[i] * np.dot(X[i], beta) - np.log(1 + np.exp(np.dot(X[i], beta)))) / n
...
def getBeta_BatchGradient(train_x, train_y, lr, num_iter, verbose):
    ...
    for iter in range(0, num_iter):
        ########## Please Fill Missing Lines Here ##########
        pred = np.array([sigmoid(np.dot(train_x[i], beta)) for i in range(n)])
        deriv = np.dot(np.transpose(train_x), train_y - pred) / n # this is deriv of logL not J ( * -1)
        # beta = beta + lr * deriv --- From before l2 regularization
        regParam = 0 # for regularization - hyperparameter - 0 obviously means no regularization
        beta = beta + lr * (deriv - 2 * regParam * beta)
...
def getBeta_Newton(train_x, train_y, num_iter, verbose):
    ...
    #beta = np.random.rand(p)
    beta = np.zeros(p) # makes unnormalized Newton method work
    
    ########## Please Fill Missing Lines Here ##########
    for iter in range(num_iter):
        temp = np.array([sigmoid(np.dot(datapoint, beta)) * (1 - sigmoid(np.dot(datapoint, beta))) for datapoint in train_x])
        hess = -1 / n * np.dot(np.multiply(np.transpose(train_x), temp), train_x)
        # perturb to make sure hessian is invertible
        perturb = 1e-20
        hess += perturb * np.identity(p)

        pred = np.array([sigmoid(y_prob) for y_prob in np.dot(train_x, beta)])
        deriv = np.dot(np.transpose(train_x), train_y - pred) / n

        beta = beta - np.dot(np.linalg.inv(hess), deriv)
